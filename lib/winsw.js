...
function createFiles(filePath, serviceName, serviceConfig) {
  ...
  var exeData = fs.readFileSync(path.join(winswOrigin, 'WinSW-x64(v2.11.0).exe'), { encoding: 'binary' });
...
function generateXml(config) {
  ...
  multi('arguments', config.execOptions || config.nodeOptions, ' ');
  xmlConfig.push({ arguments: path.resolve(config.script.trim()) });
  multi('arguments', config.scriptOptions, ' ');
