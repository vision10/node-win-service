...
module.exports = class Service extends EventEmitter {
  ...
  get winswPath() {
    var p = this.config.winswPath || this.config.deamonPath || this.config.script;
    return path.resolve(path.dirname(p), this.config.winswDirname || 'deamon');
  }
  get exists() {
  ...
  /**
   * to get any output process needs to be running as administrator
   * or installed as service otherwise it will need permision on every run
   */
