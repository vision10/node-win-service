...
module.exports = class Service extends EventEmitter {
   ...
   * @param user|username 
   * @param pass|password 
   * @param overwrite Specifies the manufacturer name of the customized executable.
   */
  install(options) {
    if (!options) { options = {} }
    try {
  ...
  async exec(command, args) {
    console.log('event', this.config.id, this.config.name, command)
    ...
    console.log('execute', winswDir + '.exe', [command].concat(args || []))
